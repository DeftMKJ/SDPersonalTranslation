...
typedef NS_OPTIONS(NSUInteger, SDWebImageOptions) {// 此种写法可以用 | 来进行配置
    /**
     ...
     * 当UIScrollView正在滚动时，延迟下载图片（避免卡顿）
...
@property (strong, nonatomic, readonly) SDImageCache *imageCache;  // 只读，实现在m文件里面
@property (strong, nonatomic, readonly) SDWebImageDownloader *imageDownloader; // 只读，实现在m文件里面
 ...
 * 改方法给指定URL弄成需要的key string 如果没有特殊设置该属性，那么[self.cacaheKeyFilter]就是空的，默认是[url absoluteString]来做key
 * @endcode
 */
 ...
 *
 * @param url            The URL to the image 图片资源
 * @param options        A mask to specify options to use for this request 指定下载设置
 * @param progressBlock  A block called while image is downloading 正在下载的时候回传block 边下载边显示
 * @param completedBlock A block called when operation has been completed. 下载完成之后回传Block
 *
